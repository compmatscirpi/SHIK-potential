...
/* ----------------------------------------------------------------------
   Original Contributing author: Eduardo Bringa (LLNL) (Buck/Coul/Cut)
------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------
   ...
   can be changed using the gamma parameters.
------------------------------------------------------------------------- */
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
 	  ...
 	  coulsmooth = exp(-(gammacoul[itype][jtype]*gammacoul[itype][jtype])/((r-rcutcoul)*(r-rcutcoul))); //Smoothening Function
          //coulsmooth_diff=2.0*(gammacoul[itype][jtype]*gammacoul[itype][jtype])/((r-rcutcoul)*(r-rcutcoul)*(r-rcutcoul));
	  ...
	  v_ljcut = a[itype][jtype]*rexpcutlj - c[itype][jtype]*r6invcutlj + d[itype][jtype]*r24invcutlj; //change
          //ljsmooth_diff = 2.0*(gammalj[itype][jtype]*gammalj[itype][jtype])/((r-rcutlj)*(r-rcutlj)*(r-rcutlj));
...
void PairSHIKWolf::coeff(int narg, char **arg)
{
  ...
  double b_one = utils::numeric(FLERR, arg[3], false, lmp); //chnage
  if (b_one <= 0) error->all(FLERR,"Incorrect args for pair coefficients");
  double c_one = utils::numeric(FLERR, arg[4], false, lmp);
  double d_one = utils::numeric(FLERR, arg[5], false, lmp); //change
  double gammalj_one = utils::numeric(FLERR, arg[6], false, lmp); //change
  double gammacoul_one = utils::numeric(FLERR, arg[7], false, lmp); //chnage
  double cut_lj_one = cut_lj_global;
  double cut_coul_one = cut_coul_global;
  if (narg >= 9) cut_coul_one = utils::numeric(FLERR, arg[8], false, lmp); //change
  if (narg == 10) cut_coul_one = utils::numeric(FLERR, arg[9], false, lmp); //change
